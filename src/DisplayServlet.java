...
public class DisplayServlet extends HttpServlet {
	 ...
	 */
	public DisplayServlet(ThreadSafeInvertedIndex index) 
	{
		super();
		this.index = index; 
	}
	...
	/**
	 * Method that properly formats the contents of the index
	 * 
	 * @param request
	 * @param response
	...
	private static void printForm(HttpServletRequest request, HttpServletResponse response, ThreadSafeInvertedIndex index) throws IOException
	{
		...
		{
			int counter = 1;
			for(var var : index.getWords())
			{
				out.printf("<p>%d. <strong>%s", counter, var, "</strong></p>%n");
				out.printf("</strong>%n");
				for(var var2 : index.getLocations(var))
				{
					out.printf("<table>%n");	
					out.printf("<a style=\"margin-left: 40px\" href=%s> %s:", var2, var2, "</a>%n");
					out.printf("</a>%n");
					out.printf("%s", index.getPositions(var, var2));
					out.printf("</table>%n");
					out.printf("</style>");
					out.printf("%n");
				}
